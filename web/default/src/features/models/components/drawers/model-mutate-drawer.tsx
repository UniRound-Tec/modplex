...
// Model metadata form. Pricing (ratios / fixed price / tiered expressions) is
// intentionally NOT edited here — it lives in the dedicated pricing sheet
// (`ModelPricingSheet`, opened via the row "Pricing" action) so there is a
// single source of truth for a model's price. Editing metadata must never
// touch the ratio option maps.
const modelMetaFormSchema = z.object({
...
type ModelMetaFormValues = z.infer<typeof modelMetaFormSchema>
...
const emptyFormValues: ModelMetaFormValues = {
  model_name: '',
  display_name: '',
  description: '',
  icon: '',
...
export function ModelMutateDrawer({
  ...
  const form = useForm<ModelMetaFormValues>({
    resolver: zodResolver(modelMetaFormSchema),
  ...
  const onSubmit = useCallback(
    async (values: ModelMetaFormValues): Promise<void> => {
