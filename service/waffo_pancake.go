...
// are exposed separately for self-service flows driven from modplex's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("modplex-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "modplex-store"
	defaultWaffoPancakeProductName = "modplex-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because modplex has no renewal-
// event handling; Pancake auto-renewing without modplex extending user
