...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/modplex/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/modplex/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/modplex/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/modplex/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/modplex/models.json", base), fmt.Sprintf("%s/api/modplex/vendors.json", base)
}
